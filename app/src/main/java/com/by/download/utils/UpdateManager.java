...
import com.by.download.downloadmanager.DownloadHandler;
import com.by.download.downloadmanager.DownloadObserver;
import com.by.download.entity.AppUpdate;
import com.by.download.listener.UpdateDialogListener;
import com.by.download.view.UpgradeDialog;
...
public class UpdateManager implements UpdateDialogListener {
     ...
     */
    public void startUpdate(Context context, AppUpdate appUpdate, DialogDismissCallBack dialogDismissCallBack) {
        ...
        bundle.putParcelable(Constants.IntentKey.updateInfo, appUpdate);
        upgradeDialog = UpgradeDialog.newInstance(bundle).addUpdateListener(this);
        upgradeDialog.show(((FragmentActivity) context).getSupportFragmentManager(), "UpdateManager");
    }
    ...
    private void downLoadApk() {
        try {
            ...
            if (context != null) {
                ...
                clearCurrentTask();
                //"http://testjiasudl.upnono.com/2.3.12_com.valvesoftware.android.steam.community.apk"
                // 下载地址如果为null,抛出异常
...
//                request.setNotificationVisibility(DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED);
     ...
     * 关闭提醒弹框
     */
    private void dismissDialog(String dismissType) {
        if (upgradeDialog != null
                && upgradeDialog.isShowing()
                && wrfContext.get() != null
                && !((Activity) wrfContext.get()).isFinishing()) {
            upgradeDialog.dismiss();
            if (this.dialogDismissCallBack != null) {
                this.dialogDismissCallBack.dismiss(dismissType);
            }
        }
    }
    ...
    public void forceExit() {
        ...
        dismissDialog(DialogDismissCallBack.forceExit);
        if (wrfContext.get() != null) {
    ...
    public void updateDownLoad() {
        ...
        if (apkFile != null) {
            ...
            if (!appUpdate.getIsSlentMode()) {
                ...
                upgradeDialog.showProgressBtn();
            } else {
                // 静默模式，不显示下载进度
                dismissDialog(DialogDismissCallBack.updateDownLoad);
            }
            // 开启下载
            downLoadApk();
        }
    ...
    public void cancelUpdate() {
        // 取消更新
        clearCurrentTask();
        dismissDialog(DialogDismissCallBack.cancelUpdate);
        if (UPGRADE_FORCE == appUpdate.getForceUpdate()) {
            forceExit();
        }
    }
    ...
    public void installApkAgain() {
        ...
        if (context != null) {
            try {
                ...
                dismissDialog(DialogDismissCallBack.installApkAgain);
            }
        }
    }
    ...
    public void installApp(File apkFile) {
        try {
            ...
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.N) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                    ...
                    if (!allowInstall) {
                        ...
                        if (upgradeDialog != null) {
                            upgradeDialog.requestInstallPermission();
                        }
                        return;
                    }
                }
            ...
            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            context.startActivity(intent);
            dismissDialog(DialogDismissCallBack.installApp);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void dismissAllDialog() {
        dismissDialog(DialogDismissCallBack.dismissAllDialog);
    }
}
